...
- (())setValue:(id)value
    ...
    // If found, invoke it with the input value (or unwrapped value, as needed) and finish.           
    ...
    // If no simple accessor is found, and if the class method accessInstanceVariablesDirectly returns YES,
    // look for an instance variable with a name like _<key>, _is<Key>, <key>, or is<Key>, in that order.
    // If found, set the variable directly with the input value (or unwrapped value) and finish.
    ...
    // These ways of accessing are kinda hacky, it'd be better if it was integrated with msg_send
    ...
    // This raises an exception by default, but a subclass of NSObject may provide key-specific behavior.
    let sel = env.objc.lookup_selector(&"setValue:forUndefinedKey:").unwrap();
    return msg_send(env, (this, sel, value, key));
}
...
forUndefinedKey:(id)key { // NSString*
    ...
    panic!("Object {:?} of class {:?} ({:?}) does not have a setter for {} ({:?})", this, class_name_string, class, key_string, key);
}
