...
// Update contentOffset and contentSize when anything that potentially affects contentSize like font and text change.
fn update_scroll(env: &mut Environment, this: id) {
    ...
    // Reset contentOffset if we have now gone out of bounds of contentSize, otherwise ignore.
...
- (())drawRect:(CGRect)_rect {
    ...
    let rect = CGRect {
        origin: CGPoint {
            x: bounds.origin.x-content_offset.x,
            y: bounds.origin.y-content_offset.y,
        },
        size: bounds.size,
    };

    let _size: CGSize = msg![env; text drawInRect:rect withFont:font lineBreakMode: UILineBreakModeTailTruncation alignment:text_alignment];
}
