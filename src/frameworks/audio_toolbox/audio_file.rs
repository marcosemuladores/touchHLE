...
pub fn AudioFileOpenWithCallbacks(
    ...
    log!("Called AudioFileOpenWithCallbacks(), limited implementation!");
    ...
    if size == 0 {
        log!("0 byte size, likely bad!");
    }
    log!("{}", size);
    let data_ptr: MutPtr<u8> = env.mem.alloc(size.try_into().unwrap()).cast();
    let bytes_read_ptr: MutPtr<u32> = env.mem.alloc(guest_size_of::<u32>()).cast();

    env.mem.write(bytes_read_ptr, 0);
    log!(
        "{:?}",
        (client_data, 0 as i64, size, data_ptr, bytes_read_ptr)
    );
    let status: OSStatus =
        read_callback.call_from_host(env, (client_data, 0 as i64, size, data_ptr, bytes_read_ptr));
    if status != 0 {
        log!(
    ...
    if env.mem.read(bytes_read_ptr) == 0 {
        log!("0 bytes read, likely bad!");
    }
...
pub const FUNCTIONS: FunctionExports = &[
    export_c_func!(AudioFileStreamOpen(_, _)),
